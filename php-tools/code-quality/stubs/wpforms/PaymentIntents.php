...
class PaymentIntents extends Common implements ApiInterface {
	 ...
	 */
	public function retrieve_payment_intent( $id, $args = array() ) {
		...
		try {
			...
			if ( isset( $args['mode'] ) ) {
				$auth_opts = array( 'api_key' => Helpers::get_stripe_key( 'secret', $args['mode'] ) );
	 ...
	 * @return bool
	 */
	public function refund_payment( string $payment_intent_id, array $args = array() ): bool {
	 ...
	 * @return Mandate|null
	 */
	public function retrieve_mandate( string $id, array $args = array() ) {
	 ...
	 */
	public function get_country_specs( string $country, array $args = array() ) {
	...
	protected function charge_single( $args ) { // phpcs:ignore Generic.Metrics.CyclomaticComplexity.TooHigh
		...
		try {
			...
			if ( isset( $args['customer_email'] ) || isset( $args['customer_name'] ) ) {
				$this->set_customer( $args['customer_email'] ?? '', $args['customer_name'] ?? '', $args['customer_address'] ?? array() );
			...
			if ( ! in_array( $this->intent->status, array( 'succeeded', 'requires_action', 'requires_confirmation' ), true ) ) {
	...
	protected function finalize_single() {
		...
		$this->intent = $this->retrieve_payment_intent( $this->payment_intent_id, array( 'expand' => array( 'customer' ) ) );
	...
	protected function charge_subscription( $args ) { // phpcs:ignore Generic.Metrics.CyclomaticComplexity.MaxExceeded
		...
		try {
			$this->set_customer( $args['email'], $args['customer_name'] ?? '', $args['customer_address'] ?? array() );

			$sub_args['customer']         = $this->get_customer( 'id' );
			$sub_args['payment_behavior'] = 'default_incomplete';
			$sub_args['off_session']      = true;
			$sub_args['payment_settings'] = array(
				'save_default_payment_method' => 'on_subscription',
			);

			if ( Helpers::is_link_supported() ) {
				$sub_args['payment_settings']['payment_method_types'] = array( 'card', 'link' );
			}
			...
			if ( ! $this->intent || ! in_array( $this->intent->status, array( 'succeeded', 'requires_action', 'requires_confirmation', 'requires_payment_method' ), true ) ) {
			...
			if ( in_array( $this->intent->status, array( 'requires_confirmation', 'requires_payment_method' ), true ) ) {
				$this->request_confirm_payment_ajax( $this->intent );
			}
	...
	private function hooks() {
		...
		add_filter( 'wpforms_process_bypass_captcha', array( $this, 'bypass_captcha_on_3dsecure_submit' ), 10, 3 );
	}
	...
	private function set_payment_element_config() {
		 ...
		 */
		$element_style = (array) apply_filters( 'wpforms_integrations_stripe_api_payment_intents_set_element_appearance', array() );
	...
	private function set_card_element_config() {
		 ...
		 */
		$element_style = (array) apply_filters( 'wpforms_stripe_api_payment_intents_set_config_element_style', array() ); // phpcs:ignore WPForms.PHP.ValidateHooks.InvalidHookName

		$this->config['localize_script']['element_style']   = $element_style;
		$this->config['localize_script']['element_classes'] = array(
