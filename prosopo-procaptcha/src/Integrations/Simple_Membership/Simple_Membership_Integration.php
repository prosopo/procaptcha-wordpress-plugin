...
class Simple_Membership_Integration extends Procaptcha_Plugin_Integration {
	...
	protected function get_conditional_form_integrations( Settings_Storage $settings_storage ): array {
		...
		return array(
			SM_Login_Form_Integration::class          => bool( $account_forms, Account_Forms_Procaptcha_Settings::IS_ON_WP_LOGIN_FORM ),
			SM_Registration_Form_Integration::class   => bool( $account_forms, Account_Forms_Procaptcha_Settings::IS_ON_WP_REGISTER_FORM ),
			SM_Reset_Password_Form_Integration::class => bool( $account_forms, Account_Forms_Procaptcha_Settings::IS_ON_WP_LOST_PASSWORD_FORM ),
		);
	}
}
